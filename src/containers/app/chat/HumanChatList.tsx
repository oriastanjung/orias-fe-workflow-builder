...
const HumanChatList = () => {
    ...
    const chats = useLiveQuery(() => db.chats.orderBy('updatedAt').reverse().filter(chat => chat.type === 'Human' && chat.title.toLowerCase().includes(search.toLowerCase())).toArray(), [search])
    ...
    const loadMore = React.useCallback(async () => {
        axiosInstance.get(`/api/v1/chat-tools/chat?type=Human&lastId=${latestId}&limit=${PAGE_SIZE}&sort=updatedAt:desc`)
            .then(res => {
    ...
    React.useEffect(() => {
        axiosInstance.get(`/api/v1/chat-tools/chat?type=Human&page=1&limit=${PAGE_SIZE}&sort=updatedAt:desc`)
            .then(async res => {
                db.chats.where('type').equals('Human').delete().then(() => {
                    db.chats.bulkPut(res.data.data)
                    if (res.data.data.length === PAGE_SIZE) {
            ...
            {chats && chats.length > 0 ? (<>
                {chats.map(chat => (
                    <div key={chat.id} onClick={() => { navigate(`/app/chat/${chat.id}`) }} className={`cursor-pointer flex flex-col gap-2 rounded-xl ${pathname.endsWith(chat.id) ? 'bg-neutral-200' : 'bg-white'} border-2 border-neutral-300 p-4 relative`}>
                        {/* <img src={chat.detail?.thumbnail as string } alt={chat.title} className='h-16 w-auto self-start rounded-sm border-2 border-white shadow-xl -rotate-5' /> */}
                ...
                {needLoadMore && (
                    <Button onClick={loadMore}>Load More Human</Button>
                )}
            </>) : (
                <div className='cursor-pointer flex flex-col items-center justify-center gap-2 rounded-xl bg-white border-2 border-neutral-300 p-4 relative' onClick={() => setIsOpen(true)}>
                    ...
                    <Plus className='' color='var(--color-neutral-500)' size={16} />
                </div>
            )}
        </div>
    )
